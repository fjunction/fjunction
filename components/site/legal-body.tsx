// components/site/legal-body.tsx  (only the wrapper's text color changed)
...
export function LegalBody({ text }: { text: string }) {
  ...
  for (const raw of text.split('\n')) {
    ...
    else { current = { type, lines: [content] }; blocks.push(current) }
  }
  ...
  return (
    <div className="space-y-4 text-sm leading-relaxed text-neutral-400">
      {blocks.map((block, i) =>
        block.type === 'ul' ? (
          <ul key={i} className="list-disc space-y-2 pl-5">{block.lines.map((line, j) => <li key={j}>{renderInline(line)}</li>)}</ul>
        ) : (
