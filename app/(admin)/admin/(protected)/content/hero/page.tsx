// app/(admin)/admin/(protected)/content/hero/page.tsx
    ...
    <div className="mx-auto max-w-2xl px-6 py-10">
      ...
      <form action={updateHero} className="mt-8 space-y-6">
        <label className="block">
          <span className="mb-1 block text-sm font-medium text-gray-700">Tag line</span>
          <input name="tag_line" defaultValue={hero?.tag_line ?? ''} className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm" />
        </label>
        ...
        <label className="block">
          ...
          <textarea name="disclaimer_text" defaultValue={hero?.disclaimer_text ?? ''} rows={2} className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm" />
        </label>

        <label className="block">
          <span className="mb-1 block text-sm font-medium text-gray-700">Photo (optional — leave blank to keep current)</span>
          {hero?.photo_url && (
            <img src={hero.photo_url} alt="Current" className="mb-2 h-32 w-44 rounded-lg object-cover" />
          )}
          <input type="file" name="photo_file" accept="image/*" className="text-sm" />
        </label>

        <label className="block">
          <span className="mb-1 block text-sm font-medium text-gray-700">Consultation note text</span>
          <input name="consultation_note_text" defaultValue={hero?.consultation_note_text ?? ''} className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm" />
        </label>
