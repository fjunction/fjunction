// app/(admin)/admin/(protected)/content/how-it-works-cta/page.tsx
import { createClient } from '@/lib/supabase/server'
import { updateHowItWorksCta } from '@/lib/admin/content-actions'

export default async function HowItWorksCtaContentPage() {
  const supabase = await createClient()
  const { data: cta } = await supabase.from('how_it_works_cta').select('*').single()

  return (
    <div className="mx-auto max-w-2xl bg-white px-6 py-10">
      <h1 className="text-2xl font-bold text-gray-900">How It Works — Clarity CTA</h1>
      <form action={updateHowItWorksCta} className="mt-8 space-y-6">
